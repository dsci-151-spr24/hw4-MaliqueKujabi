...
gss16_advfront <- gss16 %>%
  mutate(advfront = recode(advfront, 
...
```{r}
gss16_polviews_count <- gss16 %>%
  mutate(polviews = recode(polviews, 
                          "Strong Liberal" = "Liberal",
                          "Liberal" = "Liberal",
                          "Weak Liberal" = "Liberal",
                          "Strong Conservative" = "Conservative",
                          "Conservative" = "Conservative",
                          "Weak Conservative" = "Conservative"
                          )) %>%
...
```{r eval=FALSE}
set.seed(123)  
gss16_folds <- vfold_cv(gss16_rec, v = 5)  
...
gss16_subset <- gss16 %>% select(polviews, educ, advfront) %>% drop_na()

X <- gss16_subset %>% select(polviews, educ)
y <- gss16_subset$advfront

gss16_rec_2 <- recipe(advfront ~ polviews + educ, data = gss16_subset) %>%
...
cv_results_2 <- gss16_wflow_2 %>%
  fit_resamples(resamples = vfold_cv(X, v = 5))

metrics_2 <- cv_results_2 %>%
  collect_metrics()

metrics_2 %>%
  summarise(mean_roc_auc = mean(roc_auc), mean_accuracy = mean(accuracy))
```
...
Excluding wrkstat from the model resulted in a higher average AUC on the training data.
...
predictions_1 <- predict(gss16_fit_1, new_data = gss16_test) %>%
  bind_cols(gss16_test) 

predictions_2 <- predict(gss16_fit_2, new_data = gss16_test) %>%
  bind_cols(gss16_test) 

roc_1 <- roc_curve(predictions_1, truth = advfront)
roc_2 <- roc_curve(predictions_2, truth = advfront)

auc_1 <- roc_auc(roc_1)
auc_2 <- roc_auc(roc_2)

plot(roc_1, col = "blue", main = "ROC Curves for Model 1 and Model 2", lwd = 2)
plot(roc_2, col = "red", add = TRUE, lwd = 2)

legend("bottomright", legend = c(paste("Model 1 (AUC =", round(auc_1, 2), ")"),
                                 paste("Model 2 (AUC =", round(auc_2, 2), ")")),
...
response_counts <- table(harass_subset$harass5)
response_counts
```
1136=No 237=yes
...
Bootstrapping can help estimate the portion of Americans who faced harassment at work. By sampling from available responses and calculating the proportion of "Yes" answers, we can create a distribution to gauge how many Americans experienced harassment and how confident we are in that estimate.
...
n_bootstrap <- 10000
bootstrap_props <- numeric(n_bootstrap)

set.seed(123)
for (i in 1:n_bootstrap) {
  bootstrap_sample <- sample(c(rep("Yes", yes_count), rep("No", no_count)), size = total_responses, replace = TRUE)
  bootstrap_prop <- sum(bootstrap_sample == "Yes") / total_responses
  bootstrap_props[i] <- bootstrap_prop
}

conf_interval <- quantile(bootstrap_props, c(0.025, 0.975))
conf_interval
...
A 90% confidence interval would be narrower than the 95% interval, as it covers a smaller range with slightly less certainty.
